...
from conditional_RL.multiple_test import play_multiple_times_without_update_simple
        ...
        with tqdm(total=int(num_episodes/50), desc="Iteration %d" %i) as pbar:
            for i_episode in range(int(num_episodes/50)):
                ...
                if current_episode % test_freq == 0:
                    play_episodes = 100
                    test_list_vae = test_list_vae + play_multiple_times_without_update_simple(env_vae, agent_vae, all_policies_vae, 'vae', 'rule', play_episodes=play_episodes)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-l3', '--lr3', default=1e-4, help='LIAM learning rate')
