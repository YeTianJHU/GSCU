# used for adv ppo, can also be used for agent ppo, it has four layers
import torch
...
class PPO():
    ...
    def select_action(self, state, dim_c):
    ...
    for i in range(50):
        with tqdm(total=int(num_episodes/50), desc='Iteration %d' % i) as pbar:
            for i_episode in range(int(num_episodes/50)):
                ...
                for st in range(50):
                    act, act_index, action_prob = agent.select_action(obs_n[env.n-1], env.world.dim_c)
                    act_n = [] # env action
                    ...
                    trans = Transition(obs_n[env.n-1], act_index, action_prob, reward_n[env.n-1], next_obs_n[env.n-1])
